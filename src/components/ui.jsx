...
export const Icon = {
  ...
  Bag: (p) => (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.7" {...p}>
      <path d="M6 8h12l-1 12H7L6 8Z" strokeLinejoin="round" /><path d="M9 8V6a3 3 0 0 1 6 0v2" strokeLinecap="round" />
    </svg>
  ),
  User: (p) => (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.7" {...p}>
      <circle cx="12" cy="8" r="3.4" /><path d="M5 20c0-3.6 3.1-5.5 7-5.5s7 1.9 7 5.5" strokeLinecap="round" />
    </svg>
  ),
  Arrow: (p) => (
...
/* ============================================================
   ...
   ============================================================ */
export function Magnetic({ children, strength = 0.3, className = '', ...rest }) {
  ...
  return (
    <span ref={ref} className={className} onMouseMove={onMove} onMouseLeave={reset}
      style={{ display: 'inline-flex', transition: 'transform 0.5s var(--ease)' }} {...rest}>
      {children}
    </span>
  );
}
...
/* ============================================================
   Countdown to deadline (15 June 2026, 23:59 local)
   ============================================================ */
export const DEADLINE = new Date('2026-06-15T23:59:59');
...
export function CountdownGrid({ light }) {
  ...
  const items = [[d, 'Jours'], [h, 'Heures'], [m, 'Minutes'], [s, 'Secondes']];
  return (
    <div className="cd-grid">
      {items.map(([v, label], i) => (
        <React.Fragment key={label}>
          <div className="cd-cell">
            <div className="cd-num" style={{ color: light ? 'var(--paper)' : 'var(--ink)' }}>{pad(v)}</div>
            <div className="cd-label" style={{ color: light ? 'rgba(244,242,239,.55)' : 'var(--muted)' }}>{label}</div>
          </div>
          {i < 3 && <div className="cd-colon" style={{ color: 'var(--signal)' }}>:</div>}
        </React.Fragment>
      ))}
    </div>
  );
